...
class GeneratorFullModel(torch.nn.Module):
    ...
    def __init__(self, bg_predictor, dense_motion_network, inpainting_network, train_params, *kwargs):
    ...
    def forward(self, x, epoch, kp_source, kp_driving):
        """
        In this modified version, kp_source and kp_driving are passed directly as arguments
        instead of being extracted using kp_extractor.
        """
        bg_param = None
        ...
        if self.loss_weights['equivariance_value'] != 0:
            ...
            transformed_kp = kp_driving  # Use kp_driving since it's preloaded

            generated['transformed_frame'] = transformed_frame
            generated['transformed_kp'] = transformed_kp
        
            print(f"transformed_kp['fg_kp'] shape: {transformed_kp['fg_kp'].shape}")  # Check if it's [16, ...]
            # If batch size is incorrect, reshape transformed_kp['fg_kp'] to have batch size 16
            if transformed_kp['fg_kp'].shape[0] != 16:
                transformed_kp['fg_kp'] = transformed_kp['fg_kp'].view(16, -1, 2)  # Adjust according to the actual shape
