...
class InpaintingNetwork(nn.Module):
    ...
    def deform_input(self, inp, deformation):
         # Ensure inp and deformation have matching batch sizes
        if inp.size(0) != deformation.size(0):
            inp = inp.repeat(deformation.size(0), 1, 1, 1)
    ...
    def occlude_input(self, inp, occlusion_map):
        # Resize `occlusion_map` to match `inp` dimensions, regardless of `self.multi_mask`
    ...
    def forward(self, source_image, dense_motion):
        ...
        logging.info(f"Size of source_image inpainting: {source_image.shape}")

        # Reduce channels to 3 only if source_image has 84 channels
        if source_image.size(1) == 84:
            reduce_channels_to_3 = nn.Conv2d(in_channels=84, out_channels=3, kernel_size=1).to(device)
            source_image = reduce_channels_to_3(source_image)
            logging.info(f"Reduced source_image shape (for inpainting): {source_image.shape}")
        ...
        # Process deformations and occlusions on the initial `out`
        ...
        logging.info(f"Length of occlusion_map: {len(occlusion_map)}")
        ...
        for i in range(self.num_down_blocks):
            ...
            occlusion_ind = min(i + 1, len(occlusion_map) - 1)
            logging.info(f"Current occlusion_ind: {occlusion_ind}, i: {i}, num_down_blocks: {self.num_down_blocks}")
