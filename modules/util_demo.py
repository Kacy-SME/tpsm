...
class UpBlock2d(nn.Module):
    ...
    def forward(self, x):
        # Debugging statements to check input dimensions
        print(f"UpBlock2d forward - Input tensor shape: {x.shape}")
        print(f"Expected in_channels: {self.conv.in_channels}")

        # Check if channel dimensions match
        if x.shape[1] != self.conv.in_channels:
            print(f"Channel mismatch in UpBlock2d: input has {x.shape[1]} channels but expected {self.conv.in_channels}")

        # Continue with the regular forward operations
...
class AntiAliasInterpolation2d(torch.nn.Module):
    def __init__(self, num_channels, scale_factor):
        ...
        self.groups = num_channels  # Start with initial channel count
        self.ka = int((1 / self.scale - 1) / 2)
        self.kb = int((1 / self.scale - 1) / 2)

        # Initialize weight only once here based on the initial channel count
        self.init_weight(self.groups, 2 * self.ka + 1, (1 / self.scale - 1) / 2)

    def init_weight(self, groups, kernel_size, padding):
        # Adjust weights for the given groups (channels) and kernel size
        self.weight = torch.randn(groups, 1, kernel_size, kernel_size)
        # Normalize or initialize as needed

    def forward(self, input):
        if self.scale == 1.0:
            return input

        # Dynamically update `groups` and reinitialize weight if channel mismatch occurs
        if input.shape[1] != self.groups:
            self.groups = input.shape[1]
            self.init_weight(self.groups, 2 * self.ka + 1, (1 / self.scale - 1) / 2)
            logging.info(f"Reinitialized weights for groups={self.groups}")

        # Ensure weight is on the same device as input
        self.weight = self.weight.to(input.device)

        # Log details before convolution
        logging.info(f"AntiAliasInterpolation2d forward call - input shape: {input.shape}, "
                     f"weight shape: {self.weight.shape}, groups: {self.groups}")

        # Apply padding and perform depthwise convolution
        out = F.pad(input, (self.ka, self.kb, self.ka, self.kb))
        
        try:
            out = F.conv2d(out, weight=self.weight, groups=self.groups)
        except RuntimeError as e:
            logging.error(f"Error in convolution: {e}")
            raise
